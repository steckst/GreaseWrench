...
import static com.example.greasewrench.GreaseWrenchContract.*;
import static com.example.greasewrench.GreaseWrenchContract.CarInformation.*;
import static com.example.greasewrench.GreaseWrenchContract.CarMaintenanceTypes.*;

	/********************************************************
	* Class Name - GreaseWrechDBHelper, extends SQLiteOpenHelper
	* 
	* 
	* Purpose:
	*
...
public class GreaseWrechDBHelper extends SQLiteOpenHelper{
	// If you change the database schema, you must increment the database version.
	public static final int DATABASE_VERSION = 6;
	public static final String DATABASE_NAME = "GreaseWrench.db";


	public GreaseWrechDBHelper(Context context) {
	...
	* Purpose:
	...
	public void addCarInformation (CarInfo Info,SQLiteDatabase dbHandle){
		...
		if (dbHandle.isOpen()) {
			...
			values.put(COLUMN_CAR_MAKE, Info.getCarMake());
			values.put(COLUMN_CAR_MODEL, Info.getCarModel());
			values.put(COLUMN_CAR_YEAR, Info.getCarYear());
			values.put(COLUMN_CAR_MILEAGE, Info.getCarMileage());
			values.put(COLUMN_CAR_IMAGE, " ");
	...
	public void addMaintenanceType(CarMaintTypes Type,SQLiteDatabase dbHandle){
		...
		if (dbHandle.isOpen()) {
			try {
				ContentValues values = new ContentValues();
				// Constants below are defined in GreaseWrenchContract.CarInformation.
				values.put(COLUMN_MAINTENANCE_TYPE, Type.getMaintenanceType());
